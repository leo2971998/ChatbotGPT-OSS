  ...
  return (
      <div className="weather-card">
        ...
        <div className="wc-main">
          <div className="wc-temp">{Number.isFinite(temperature) ? Math.round(temperature) : "—"}°</div>
          <div className="wc-feels">
            Feels {Number.isFinite(feelsLike) ? Math.round(feelsLike) : "—"}°
          </div>
        </div>
        <div className="wc-right">
          <div>
            H:{Number.isFinite(high) ? Math.round(high) : "—"}° / L:
            {Number.isFinite(low) ? Math.round(low) : "—"}°
          </div>
          <div>Humidity: {Number.isFinite(humidity) ? Math.round(humidity) : "—"}%</div>
          <div>Wind: {Number.isFinite(wind) ? Math.round(wind) : "—"} km/h</div>
          <div>UV: {uv ?? "—"}</div>
          <Legend />
        </div>
      </div>
